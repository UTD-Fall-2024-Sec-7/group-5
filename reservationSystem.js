...
function addReservation() {
    ...
    if (!/^[A-Za-z]+$/.test(name)) {
        console.log("Invalid name, please only use alphabetical letters");
        return;
    }
    ...
    if (!/^([0-1]\d|2[0-3]):([0-5]\d)$/.test(time)) {
        console.log("Invalid time, please use the format HH:MM");
        return;
    }
    if (isNaN(guests) || guests < 1) {
        console.log("Invalid number of guests, please enter a number greater than 1");
        return;
    }
...
function sortReservations() {
    ...
    switch (sortBy) {
        ...
        case "guests":
            ...
            break;
        default:
            console.log("Invalid option, please enter date, time, or guests");
            return;
    }
    viewReservations(); 
}
